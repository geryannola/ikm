...
<?php    $no=1;
			...
			$query="select * from responden a, matrix b where a.id_s=b.id_s group by b.id_s";
...
<td>Total</td>
			  ...
			  ?>
                 ...
                 <?php
			  $nsql="select SUM(nilai) AS nrr from matrix where id_u=$id_u ";
...
<td> Rata-Rata</td>
  <?php  
			  $total=0;
			  $responden=mysql_num_rows(mysql_query("select * from responden"));
			  $sql="select * from unsur ";
			  $query=mysql_query($sql);
			  while($data=mysql_fetch_array($query)){ 
			  $id_u=$data['id_u'];                         
