...
<?php    $no=1;
			...
			$query="select * from tbl_pesan a, matrix b where a.id_s=b.id_s and b.id_kat=$id_kat and a.tahun='$tahun' and a.tahun=b.tahun group by b.id_s";
...
<!--isi colom tabel dengan nilai hasil query sql di atas-->
                <tr id="col">
                    <td style="text-align:center;" class="fname"><?php echo $no;?></td>
					<?php 
					$nilai2=0;
                    $matrix="select * from  matrix  where id_s=$id_s and id_kat=$id_kat and tahun=$tahun";
...
<td>Total</td>
			  ...
			  ?>
                 ...
                 <?php
			  $nsql="select SUM(nilai) AS nrr from matrix a, tbl_pesan b where a.id_p=$id_p and a.id_s=b.id_s and a.id_kat=$id_kat and a.id_kat=b.id_kat and b.tahun='$tahun'  and a.tahun=b.tahun ";
...
<td> Rata-Rata</td>
  <?php  
			  $total=0;
			  $responden=mysql_num_rows(mysql_query("select * from tbl_pesan where id_kat=$id_kat and tahun='$tahun' "));
			  $sql="select * from tbl_pertanyaan where id_kat=$id_kat ";
			  $query=mysql_query($sql);
			  while($data=mysql_fetch_array($query)){ 
			  $id_p=$data['id_p'];                         
			  ?>
                 ...
                 <?php
			  $nsql="select SUM(nilai) AS nrr from matrix a, tbl_pesan b where a.id_kat=$id_kat and a.id_p=$id_p and  a.id_kat=b.id_kat and a.id_s=b.id_s and b.tahun='$tahun' and a.tahun=b.tahun ";
			  $nquery=mysql_query($nsql);
			  while($ndata=mysql_fetch_array($nquery)){    
...
<td> Rata-Rata Tertimbang</td>
  <?php  
			  ...
			  ?>
                 ...
                 <?php
			  $nsql="select SUM(nilai) AS nrr from matrix a, tbl_pesan b where a.id_p=$id_p and a.id_kat=$id_kat and a.id_kat=b.id_kat and a.id_s=b.id_s and b.tahun='$tahun' and a.tahun=b.tahun  ";
			  $nquery=mysql_query($nsql);
			  while($ndata=mysql_fetch_array($nquery)){    
...
<?php } ?>
 ...
 <div><b> Export</b> <a target="_blank" href="pdf.php?&kat=<?php echo $id_kat;?>&tahun=<?php echo $tahun;?>"> Testing</a></div>
